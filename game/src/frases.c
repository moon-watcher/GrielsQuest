...
const static FRASE _list [ ] =
{
	...
	(FRASE) { SPANISH,  1, "NIVEL %02d" },


	(FRASE) { SPANISH,  2, "\\ CONTINUAR" },     // 0
	(FRASE) { SPANISH,  2, "  REINICIAR" },      // 1
	(FRASE) { SPANISH,  2, "  SALIR" },          // 2
	(FRASE) { SPANISH,  2, "REINICIAR NIVEL" },  // 3
	(FRASE) { SPANISH,  2, "SALIR DE NIVEL" },   // 4
	(FRASE) { SPANISH,  2, "NIVEL COMPLETADO" }, // 5
	...
	(FRASE) { SPANISH,  5, "FRÍO" }, // 21
	(FRASE) { SPANISH,  5, "CONGELADO" }, // 22
	(FRASE) { SPANISH,  5, "BAJO CERO" }, // 23

	//
	// excuses
	(FRASE) { SPANISH,  6, "Siempre me engaña el lobo." },                           // woods
	(FRASE) { SPANISH,  6, "Hielo para los refrescos." },                            // iceland
	(FRASE) { SPANISH,  6, "La arena se me mete en la coquilla." },                  // egypt
	(FRASE) { SPANISH,  6, "No llevo traje de baño." },                              // beach
	//
	(FRASE) { SPANISH,  6, "Here's Johnny!" },                                                     // The shinning ref.
	(FRASE) { SPANISH,  6, "Estos no son los androides que buscáis." },                            // Starwars ref
	(FRASE) { SPANISH,  6, "No puedo ir ahí, es el triángulo prohibido." },                        // Monkey Island 2 refference
	(FRASE) { SPANISH,  6, "No puedo ir ahí, es el dodecaedro prohibido." },                       // Monkey Island 2 refference
	(FRASE) { SPANISH,  6, "RISE FROM YOUR GRAVE!" },                                              // Altered beast refference
	(FRASE) { SPANISH,  6, "Mira detrás de ti, ¡un mono con tres cabezas!" },                      // Monkey Island 2 refference
	(FRASE) { SPANISH,  6, "¿Llevas un plátano en el bolsillo o es que te alegras de verme?" },    // Monkey Island 2 refference
	(FRASE) { SPANISH,  6, "¡Más potencia, Scotty!"   },                                           // Star trek refference
	(FRASE) { SPANISH,  6, "¿Cuántos Klingon hacen falta para cambiar una bombilla?" },            // Joke, Star trek refference
	(FRASE) { SPANISH,  6, "¿Saben aquel que diu...?" },                                           // Eugenio LoL
	(FRASE) { SPANISH,  6, "Ve al castillo de Kbrah." },                                           //
	(FRASE) { SPANISH,  6, "¡Oh, venga ya! No tengo todo el día." },                               //
	(FRASE) { SPANISH,  6, "¿No has curioseado suficiente?" },                                     //
	//
...
//	(FRASE) { SPANISH, 10, "Griel, sin el poder del Holy Porrón el reino está condenado. |WBREAK ¡Debes recuperarlo cuanto antes!"},//  "Griel, han robado el Holy Porrón, su poder es temible. ¡Debes recuperarlo cuanto antes!" },
//	(FRASE) { SPANISH, 10, "Esto apesta a mi archienemigo Kbrah. |WBREAK ¡Seguro que ha sido él!" }, // "Kbrah me la tiene jurada. |BREAK Además el salón apesta a ese truhán, seguro que ha sido él. |WCLEAR Encuentra a Kbrah y trae de vuelta el Porrón." },
...
//	(FRASE) { SPANISH, 23, "-LONGITUD MAXIMA DISPONIBLE-" }, // 28 MAX
    ...
    (FRASE) { SPANISH, 23, "Supera todos los mundos" },
    (FRASE) { SPANISH, 23, "para llegar hasta Kbrah." },

    (FRASE) { SPANISH, 23, "Aprende a abrirte paso" },
    (FRASE) { SPANISH, 23, "entre los enemigos." },

    (FRASE) { SPANISH, 23, "Usa el arma adecuada en" },
    (FRASE) { SPANISH, 23, "cada ocasión." },
    ...
    (FRASE) { SPANISH, 23, "Si caes, te quemas, o te " },
    ...
    (FRASE) { SPANISH, 23, "superas TODOS los niveles." },

    (FRASE) { SPANISH, 23, "En caso de duda consulte" },
    (FRASE) { SPANISH, 23, "con su farmacéutico." },

    (FRASE) { SPANISH, 23, "Griel y el Rey Notah cuentan" },
    (FRASE) { SPANISH, 23, "contigo ¡No les falles!." },
    ...
    (FRASE) { SPANISH, 23, "@2049" },


    // How to play --------------------------------------------------------------------------
    (FRASE) { SPANISH, 24, " ENEMIGOS " }, // ojo a los espacios delante y detrás
    (FRASE) { SPANISH, 24, " OBJETOS "  }, // ojo a los espacios delante y detrás


    // Game over ----------------------------------------------------------------------------
    (FRASE) { SPANISH, 25, "GAME OVER" },
    (FRASE) { SPANISH, 25, "Usa este password" },
    (FRASE) { SPANISH, 25, "¡Gracias por jugar!" },
    (FRASE) { SPANISH, 25, "Modo invertido" },
    ...
    (FRASE) { SPANISH, 27, "CONTINUAR" },
    (FRASE) { SPANISH, 27, "TEST DE SONIDO" },

    // Sound test ---------------------- ----------------------------------------------------
    (FRASE) { SPANISH, 28, "TEST DE SONIDO" },
    (FRASE) { SPANISH, 28, "Volver" },

    // --------------------------------- ----------------------------------------------------
    (FRASE) { SPANISH, 29, "PASSWORD" }, // LEAVE AS IS UNLESS TRANSLATION WERE SAME LENGTH

    // - Password screen --------------- ----------------------------------------------------
    (FRASE) { SPANISH, 30, "CONTRASEnA" }, // n is Ñ in bigfont


    // HOW TO PLAY // WEAPONS
    (FRASE) { SPANISH, 31, "Bastón" },
    (FRASE) { SPANISH, 31, "Cruz" },
    (FRASE) { SPANISH, 31, "Espada" },
    (FRASE) { SPANISH, 31, "Llave" },

    // HOW TO PLAY // ENEMIES
    (FRASE) { SPANISH, 32, "Jabalí" },
    (FRASE) { SPANISH, 32, "Seta" },
    (FRASE) { SPANISH, 32, "Elfa" },

    (FRASE) { SPANISH, 33, "Morsa" },
    (FRASE) { SPANISH, 33, "Eskimo" },
    (FRASE) { SPANISH, 33, "Wampa" },

    (FRASE) { SPANISH, 34, "Swordman" },
    (FRASE) { SPANISH, 34, "Escorpión" },
    (FRASE) { SPANISH, 34, "Momia" },

    (FRASE) { SPANISH, 35, "Cangrejo" },
    (FRASE) { SPANISH, 35, "Pirata" },
    (FRASE) { SPANISH, 35, "Plesi" },

    (FRASE) { SPANISH, 36, "Ogro" },
    (FRASE) { SPANISH, 36, "Ojo" },
    (FRASE) { SPANISH, 36, "Demonio" },

    (FRASE) { SPANISH, 37, "Puerta" },
...
static void _prepare_string ( u8 *str )
{
	...
	for ( i = 0; str [ i ] != '\0'; i++ )
	{
		...
		switch ( _string [ i ] )
		{
			case 250: _string [ i ] = 32 -  1;  break; // ú
			case 243: _string [ i ] = 32 -  2;  break; // ó
			case 241: _string [ i ] = 32 -  3;  break; // ñ
			case 237: _string [ i ] = 32 -  4;  break; // í
			case 233: _string [ i ] = 32 -  5;  break; // é
			case 225: _string [ i ] = 32 -  6;  break; // á
			case 218: _string [ i ] = 32 -  7;  break; // Ú
			case 211: _string [ i ] = 32 -  8;  break; // Ó
			case 209: _string [ i ] = 32 -  9;  break; // Ñ
			case 205: _string [ i ] = 32 - 10;  break; // Í
			case 201: _string [ i ] = 32 - 11;  break; // É
			case 193: _string [ i ] = 32 - 12;  break; // Á
			case 191: _string [ i ] = 32 - 13;  break; // ¿
			case 161: _string [ i ] = 32 - 14;  break; // ¡
		}
	}
